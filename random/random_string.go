...
// 确保 Hash != nil
func commonRandomString(Hash hash.Hash, localSalt []byte, otherSalts []string) []byte {
...
//  Hash = md5.New() if nil
func NewRandomString(Hash hash.Hash, salts ...string) []byte {
	if Hash == nil {
		Hash = md5.New()
	}
	return commonRandomString(Hash, localRandomSalt, salts)
}

// The returned bytes has been hex encoded.
//  Hash = md5.New() if nil
func NewTokenString(Hash hash.Hash, salts ...string) []byte {
	if Hash == nil {
		Hash = md5.New()
	}
	token := commonRandomString(Hash, localTokenSalt, salts)
...
func NewSessionIDString(salts ...string) []byte {
